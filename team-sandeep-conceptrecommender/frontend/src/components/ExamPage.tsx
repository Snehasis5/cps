...
interface ExamPageProps {
  topic: Topic; // topic.name is the unique identifier
  onBack: () => void;
  onComplete: (result: ExamResultSubmission) => void;
  questions?: ExamQuestion[]; // Optional: allow passing questions from parent
}
...
export const ExamPage: React.FC<ExamPageProps> = ({ 
  ...
  onBack, 
  onComplete,
  questions: propQuestions // new optional prop
}) => {
  ...
  const [questions, setQuestions] = useState<ExamQuestion[]>(propQuestions || []);
  ...
  useEffect(() => {
    ...
    console.log('Requesting random quiz for topic:', topic.name);
  ...
  const handleSubmitExam = async () => {
    ...
    // Send result to backend and only update UI if successful
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        setError('You are not logged in. Please log in again.');
      ...
      if (!response.ok) {
        let errorMsg = 'Failed to save exam result';
        try {
      ...
      setScore(finalScore);
      setShowResults(true);
      // Pass all required fields to onComplete
      onComplete({ score: finalScore, totalQuestions, correctAnswers, timeTaken });
    } catch (err) {
      const msg = (err instanceof Error && err.message) ? err.message : 'Failed to save exam result!';
      setError(msg);
      alert(msg);
    }
  };
  ...
  if (questions.length === 0) {
    return <div className="p-8 text-center text-gray-500">No questions available for this topic.</div>;
  }
